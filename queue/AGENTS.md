...
- **BLOCKER**: Queue capacity equals the number of active queue-worker automations targeting this repository. Count `[ ]` and `[-]` as outstanding work. With the current single worker automation, capacity is one row.
- **BLOCKER**: The reviewer may create the queue only when at least one validated `[ ]` row exists and capacity is available. Never create an empty queue or exceed capacity. With one worker, never append while the queue exists; wait for the worker to finish and delete it.
